    ...
    with pymysql.connect(host=DB_HOST, user=DB_USER, password=DB_PASSWORD, db=DB_DATABASE) as conn:
        with conn.cursor() as cursor:
            try:
                ...
                query = f'INSERT INTO sdk_mms_send(USER_ID,SCHEDULE_TYPE,SUBJECT,NOW_DATE,SEND_DATE,CALLBACK,DEST_COUNT,DEST_INFO,MSG_TYPE,MMS_MSG,CONTENT_COUNT,CONTENT_DATA) values("raon",0,"Control Message Alarm",DATE_FORMAT(NOW(),"%Y%m%d%H%i%s"),DATE_FORMAT(NOW(),"%Y%m%d%H%i%s"),"0428788933",1,"{dest_info}",0,"{message}",0,"")'
